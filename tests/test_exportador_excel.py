...
class ExportadorExcelTestCase(unittest.TestCase):
    def test_deve_gerar_workbook_com_abas_esperadas(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as diretorio_temporario:
            ...
            self.assertEqual(workbook["ranking_final"].auto_filter.ref, "A1:I2")
            self.assertEqual(workbook["ranking_final"].sheet_properties.tabColor.rgb, "002FD126")
            self.assertEqual(workbook["avaliacao_por_criterio"].sheet_state, "hidden")
            self.assertEqual(workbook["pendencias_revisao"].sheet_state, "hidden")
            ...
            self.assertTrue(workbook["ranking_final"]["I2"].alignment.wrap_text)
            workbook.close()
